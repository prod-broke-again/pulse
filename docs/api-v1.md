...
|-------------|--------|-----------------------------------------------|
| tab         | string | `my` \| `unassigned` \| `all` (по умолчанию; legacy) |
| assigned_to_me | bool  | Только назначенные на текущего модератора     |
| unassigned_only | bool | Только свободные (нераспределённые) чаты      |
| unread_only | bool   | Только чаты с непрочитанными сообщениями от клиентов |
| chat_status | string | Фильтрация по точному статусу открытого чата: `new` \| `active` |
| source_id   | int    | Фильтр по источнику                           |
| source_ids[] | int   | Фильтр по списку источников                   |
| department_id | int  | Фильтр по отделу                              |
| department_ids[] | int | Фильтр по списку отделов                      |
...
### GET /chats/inbox-summary

Суммарная сводка открытых и непрочитанных обращений по всем привязанным отделам. Принимает те же фильтры, что и `GET /chats`, но игнорирует параметры `department_id`, `department_ids` и `tab`, чтобы посчитать статистику по всем отделам параллельно.
...
```json
{
  "data": {
    ...
    "departments": [
      {
        "id": 1,
        "name": "АЧПП ID: вход и регистрация",
        "open": 10,
        "unread": 2
      },
      {
        "id": 3,
        "name": "Аврора: консультации",
        "open": 12,
        "unread": 2
      }
    ]
