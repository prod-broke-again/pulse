...
return new class extends Migration
{
    ...
    {
        if (! Schema::hasColumn('canned_responses', 'owner_user_id')) {
        ...
        if (Schema::hasColumn('canned_responses', 'source_id')) {
        ...
        if (! Schema::hasColumn('quick_links', 'owner_user_id')) {
        ...
        if (Schema::hasColumn('quick_links', 'source_id')) {
    ...
    public function down(): void
    {
        ...
        if (Schema::hasColumn('canned_responses', 'scope_type')) {
            DB::table('canned_responses')->orderBy('id')->chunkById(100, function ($rows): void {
                foreach ($rows as $row) {
        ...
        if (Schema::hasColumn('quick_links', 'scope_type')) {
            DB::table('quick_links')->orderBy('id')->chunkById(100, function ($rows): void {
                foreach ($rows as $row) {
                    /** @var object{id:int,scope_type:string|null,scope_id:int|null} $row */
                    if ($row->scope_type === 'source' && $row->scope_id !== null) {
